...
def prepare_data_train(path, work_dir):
	...
	train.to_csv(Path.joinpath(work_dir,'train_copy.csv'), index=False)
	train["Timestamp"] = train["Timestamp"].astype("datetime64[ns]")

	train = train.set_index("MessageId")
	train = train.sort_values(by=['Timestamp'])

	train['uniq'] = train['BeneficiaryAccount'] + train['Receiver']
	df4 = train[~train['uniq'].isin(train.loc[train['Label'].eq(0), 'uniq'])]
	unknown_accounts = df4[['Receiver','BeneficiaryAccount']].drop_duplicates()
	#print(len(df4))
	#unknown_accounts = train[['Receiver','BeneficiaryAccount']].drop_duplicates()
	unknown_accounts.to_csv(Path.joinpath(work_dir, "unknown_accounts.csv"), index=False)
	#train.drop(['uniq'], axis=1)
	...
	join_keys = ['MessageId', 'Timestamp']

	start_time = time.time()

	keys = ["Sender", "Receiver", "InstructedCurrency", "SettlementCurrency"]
	fields = ["InstructedAmount", "SettlementAmount", "Timestamp"]
	df = train.groupby(keys)[fields].rolling('28D', closed="left", on="Timestamp").max()
	rename_dict = {item:item+"bank_pair_max" for item in fields if item not in ["Timestamp"]}
	df = df.rename(rename_dict, axis="columns")
	train = train.merge(df, left_on=join_keys, right_on=join_keys, how="left")
	
	df = train.groupby(keys)[fields].rolling('28D', closed="left", on="Timestamp").min()
	rename_dict = {item:item+"bank_pair_min" for item in fields if item not in ["Timestamp"]}
	df = df.rename(rename_dict, axis="columns")
	train = train.merge(df, left_on=join_keys, right_on=join_keys, how="left")

	df = train.groupby(keys)[fields].rolling('28D', closed="left", on="Timestamp").mean()
	rename_dict = {item:item+"bank_pair_mean" for item in fields if item not in ["Timestamp"]}
	df = df.rename(rename_dict, axis="columns")
	train = train.merge(df, left_on=join_keys, right_on=join_keys, how="left")


	keys = ["OrderingName", "BeneficiaryName", "InstructedCurrency", "SettlementCurrency"]
	fields = ["UETR", "Timestamp"]
	...
	train = train.rename(columns={'index':'MessageId'})
	...
	pd_train = train
	pd_train['Label'].to_csv(Path.joinpath(work_dir,"train_label.csv"))
	...
	pd_minority_upsampled = resample(pd_minority,
							 replace=True,
							 n_samples=int(len(pd_majority)/RESAMPLE_RATIO),
							 random_state=42)
	...
	pd_train_temp[['MessageId', 'Label']].to_csv(Path.joinpath(work_dir,'train_label.csv'), index=False)
	pd_train_temp.drop(columns=columns_to_drop, inplace=True)
	pd_train_temp.to_csv(Path.joinpath(work_dir,'train_data_processor.csv'), index=False)
...
def prepare_data_test(path, work_dir):
	...
	unknown_accounts = test[['Receiver','BeneficiaryAccount']].drop_duplicates()
	unknown_accounts.to_csv(Path.joinpath(work_dir, "unknown_accounts.csv"), index=False)


	train = pd.read_csv(Path.joinpath(work_dir,'train_copy.csv'))
	test["Timestamp"] = test["Timestamp"].astype("datetime64[ns]")
	train["Timestamp"] = train["Timestamp"].astype("datetime64[ns]")
	actual_test["Timestamp"] = actual_test["Timestamp"].astype("datetime64[ns]")
	if TEST_MODE:
		test = pd.concat([train.drop(columns=["Label"]), test])
	else:
		test = pd.concat([train, test])

	#print(test.columns)
	#print(test.index.is_unique)
	#print(test.index)
	#print(test.columns.is_unique)
	test = test.set_index('MessageId')
	print(test.index.is_unique)
	test["Timestamp"] = test["Timestamp"].astype("datetime64[ns]")
	...
	keys = ["Sender", "Receiver", "InstructedCurrency", "SettlementCurrency"]
	fields = ["InstructedAmount", "SettlementAmount", "Timestamp"]
	df = test.groupby(keys)[fields].rolling('28D', closed="left", on="Timestamp").max()
	rename_dict = {item:item+"bank_pair_max" for item in fields if item not in ["Timestamp"]}
	df = df.rename(rename_dict, axis="columns")
	test = test.merge(df, left_on=join_keys, right_on=join_keys, how="left")

	df = test.groupby(keys)[fields].rolling('28D', closed="left", on="Timestamp").min()
	rename_dict = {item:item+"bank_pair_min" for item in fields if item not in ["Timestamp"]}
	df = df.rename(rename_dict, axis="columns")
	test = test.merge(df, left_on=join_keys, right_on=join_keys, how="left")

	df = test.groupby(keys)[fields].rolling('28D', closed="left", on="Timestamp").mean()
	rename_dict = {item:item+"bank_pair_mean" for item in fields if item not in ["Timestamp"]}
	df = df.rename(rename_dict, axis="columns")
	test = test.merge(df, left_on=join_keys, right_on=join_keys, how="left")
	...
	test = test.rename(columns={'index':'MessageId'})
	test = actual_test[['MessageId']].merge(test, left_on=["MessageId"], right_on=["MessageId"], how="left")
	...
	if TEST_MODE:
		pd_test.to_csv(Path.joinpath(work_dir, 'test_data_processor.csv'), index=False)
	else:
		columns_to_drop = ["Label"]
		pd_test[['MessageId','Label']].to_csv(Path.joinpath(work_dir,'test_label.csv'), index=False)
		pd_test.drop(columns=columns_to_drop, inplace=True)
		pd_test.to_csv(Path.joinpath(work_dir,'test_data_processor.csv'), index=False)


def prepare_bank_data(bank_path, work_dir):
	#bank_path, work_dir = str(bank_path), str(work_dir)
	### Bank Data
	data_bank = pd.read_csv(bank_path)

	banks = data_bank.Bank.drop_duplicates().to_list()
	...
	#
	bankFlagDict = dict(zip(zip(data_bank.Bank, data_bank.Account), data_bank.Flags.eq(0)))
	torch.save(bankFlagDict, Path.joinpath(work_dir, "bank_flag_dict"))
	return banks
